...
## D046 - Criterio documental TEC-009 para memoria beta

Se decide que TEC-009 debe redactarse en la memoria como:

- `exfiltracion HTTP controlada en laboratorio`; o
- `staging, archivado ZIP y exfiltracion HTTP controlada hacia host receptor`.

Debe evitarse cualquier formulacion que rebaje TEC-009 a mera preparacion
local o invalide el envio HTTP documentado por el summary.

La evidencia defendible para la memoria es:

- ZIP generado;
- datos dummy en staging;
- envio por red a otro host;
- transferencia HTTP controlada;
- accion equivalente a Invoke-WebRequest / Invoke-RestMethod / upload HTTP;
...
mantiene como limitacion documental secundaria sobre conservacion del receptor
final, no como invalidacion de TEC-009.

Justificacion:

- La tecnica ejecuta staging, archivado y transferencia HTTP controlada.
- La ausencia de un artefacto receptor conservado en el paquete documental no
  invalida el summary con HTTP 200, upload correcto, hash y bytes transferidos.
- La memoria debe separar ejecucion, visibilidad, deteccion, salida externa y
  evidencia forense sin degradar incorrectamente TEC-009.

## D047 - Referencias y anexos de memoria beta

Se decide completar la beta de memoria directamente en:
...
- Citas en el cuerpo para fuentes externas oficiales e internas del TFM.
- Anexos integrados en el propio Word, no como documentos auxiliares nuevos.
- Las fuentes internas se citan para trazabilidad de resultados, no como
  sustituto de bibliografia externa.
- Los anexos no duplican evidencias brutas; resumen trazabilidad y apuntan a
  Exceles, normalizados, artifacts y documentos de contexto.

Controles:

- Word abrio, actualizo indices/listas y guardo la beta.
- Se mantiene TEC-009 como exfiltracion HTTP controlada en laboratorio.
- Se mantiene CLIENT_EVENT como capa de deteccion defendible.
- SERVER_EVENT queda como capa de enrutamiento, normalizacion, persistencia o
  notificacion.
- JSONL y Discord quedan como salidas externas.
- Benchmark queda como coste operativo, no calidad de deteccion.

## D048 - Dictamen pre-PDF memoria beta

Se decide que `TFM_MEMORIA_BETA_v1.docx` no debe exportarse todavía como PDF
final.

Estado de revisión:

- `APTO PARA REVISIÓN FINAL HUMANA`;
- no apto para PDF final;
- cierre estimado: 88 %.

Motivos:

- índice general duplicado antes del índice de código;
- índice de tablas desactualizado;
- frase del capítulo VI sobre JSONL/Discord puede contradecir que son salida
  externa, no fuente primaria de detección;
- falta revisión visual humana de tablas, captions, saltos de página y anexos.

Se permite trabajar sobre:

- `TFM_MEMORIA_BETA_v1_REVISION_PREPDF.docx`.

No se debe tocar el original salvo decisión explícita posterior.

Se mantiene:

- TEC-009 como exfiltración HTTP controlada en laboratorio;
- `CLIENT_EVENT` como unidad primaria de detección;
- `SERVER_EVENT` como enrutamiento/normalización/persistencia/notificación;
- JSONL y Discord como salida externa;
- benchmark como coste operativo;
- Wazuh como comparación HIDS/SIEM no equivalente.
