...
## Beta documental de memoria (2026-07-08 11:55 CEST)

- Regenerada nueva version beta de trabajo desde el documento real mas
  actualizado:
  `08_MEMORIA/TFM.docx`.
...
- Se archiva la beta previa y documentos auxiliares antiguos en:
  `08_MEMORIA/ITERACION_PROXIMA_MEMORIA_10_10/_ARCHIVO_OBSOLETO_NO_USAR`.
- Nuevos documentos auxiliares:
...
- Correccion metodologica principal:
  TEC-009 queda formulada como staging, archivado ZIP y exfiltracion HTTP
  controlada en laboratorio. La ausencia de `receiver_log.jsonl` o ZIP recibido
  en el paquete revisado queda como limitacion documental secundaria, no como
  invalidacion de la tecnica.
- Actualizada la tabla comparativa Wazuh - Velociraptor:
  se elimina `Pendiente de campana VR` y se conserva Wazuh 110201 como gap.
- Se refuerza que benchmark mide coste operativo, no calidad de deteccion.
- Validacion local documental:
  `.docx` abre como paquete OpenXML, `word/document.xml` parsea correctamente,
  110 entradas en el paquete y busqueda sin frases obsoletas de invalidacion
  TEC-009.
- No se modifican artifacts, scripts, runners, evidencias ni Exceles.
- No se ejecutan pruebas experimentales.
- Estado superado por la actualizacion 2026-07-08 12:50: referencias APA,
  anexos e indices/listas ya fueron incorporados en la beta; queda revision
  visual humana.

## Beta documental de memoria - referencias y anexos (2026-07-08 12:50 CEST)

- Actualizado el entregable principal:
  `08_MEMORIA/ITERACION_PROXIMA_MEMORIA_10_10/TFM_MEMORIA_BETA_v1.docx`.
- Referencias bibliograficas completadas en APA.
- Citas incorporadas en el cuerpo para:
  - MITRE ATT&CK Enterprise y tecnicas TEC-001 a TEC-009;
  - Velociraptor artifacts, VQL, event queues y server monitoring;
  - Microsoft Sysmon y PowerShell Logging;
  - Wazuh agent, custom rules, indexer/log collection;
  - fuentes internas del TFM: Excel de visibilidad, Excel benchmark, artifacts
    validados, reglas Wazuh y documentos de contexto.
- Anexos A-L desarrollados:
  tecnicas MITRE, laboratorio, telemetria/evidencia, artifacts, runners,
  datasets, resultados, benchmark, Wazuh, TEC-009, glosario y checklist
  metodologico.
- Word abrio la beta en modo invisible, actualizo indices/listas y guardo el
  documento sin error.
- SHA-256 final beta:
  `DFB82ECF760E56C9F8E8D78485CF7D9571DCB6C6254D6281C9BF3BB95D9025C3`.
- No se modifican artifacts, scripts, runners, evidencias ni Exceles.
- No se ejecutan pruebas experimentales.
- Pendiente revision visual humana de paginacion, tablas, captions, anexos y
  exportacion PDF.

## Revisión final pre-PDF memoria beta (2026-07-09 18:20 CEST)

- Revisada la memoria beta como revisor académico, técnico y metodológico.
- Original:
  `08_MEMORIA/ITERACION_PROXIMA_MEMORIA_10_10/TFM_MEMORIA_BETA_v1.docx`.
...
- Correcciones menores aplicadas solo en la copia:
  tildes, `Discord`, captions visibles, eliminación de `Final del formulario`
  y títulos del capítulo IX.
- Bloqueantes antes de PDF:
  índice general duplicado, índice de tablas desactualizado, frase JSONL/Discord
  en capítulo VI y revisión visual de tablas/anexos.
- TEC-009 se mantiene correctamente como exfiltración HTTP controlada en
  laboratorio.
- Benchmark se mantiene como coste operativo, no calidad de detección.
- Wazuh se mantiene como comparación HIDS/SIEM no equivalente.
- No se modificaron artifacts, scripts, runners, evidencias ni Exceles.
- No se ejecutaron pruebas experimentales.

## Resumen de contexto ChatGPT/Codex pre-PDF (2026-07-09 18:25 CEST)

- Creado documento de contexto rápido:
  `00_CONTEXT/RESUMEN_CONTEXTO_CHATGPT_CODEX_PREPDF.md`.
- Objetivo:
  permitir a ChatGPT/Codex recuperar rápidamente el estado de la memoria beta
  pre-PDF, lo realizado, lo pendiente, lo encontrado y los criterios
  metodológicos fijos.
- El documento incluye:
  ...
  - cambios aplicados en `REVISION_PREPDF`;
  - hallazgos P1/P2/P3;
  - checklist exacto antes de PDF;
  - qué no hacer.
- No se modificó la memoria Word en esta sesión.
- No se modificaron artifacts, scripts, runners, evidencias ni Exceles.
