...
$scriptPath = Join-Path $root 'scripts\final_cleanup_word.ps1'
...
& $task `
    ...
    -LogPath (Join-Path $root 'qa\final_cleanup_word.json') `
    -ProgressPath (Join-Path $root 'qa\final_cleanup_word.progress.txt')
