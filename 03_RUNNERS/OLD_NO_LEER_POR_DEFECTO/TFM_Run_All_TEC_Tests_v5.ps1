...
<#
Candidate runner v5 for TFM Velociraptor validation.
...
function Test-Tec009Evidence {
    ...
    $checks.Add((New-EvidenceCheckRow -Name "powershell.exe -File invocation" -Status $(if ($Command -match '(?i)powershell\.exe\s+-NoProfile\s+-ExecutionPolicy\s+Bypass\s+-File') { "OK" } else { "FAIL" }) -Evidence $Command -Comment "TEC-009 must run in an explicit PowerShell child process.")) | Out-Null
    $checks.Add((New-EvidenceCheckRow -Name "TEC-009 script exists" -Status $(if (Test-Path -LiteralPath $ScriptPath) { "OK" } else { "FAIL" }) -Evidence $ScriptPath -Comment "The runner must invoke the real exfiltration script path.")) | Out-Null
    $summaryExists = (-not [string]::IsNullOrWhiteSpace($SummaryFile)) -and (Test-Path -LiteralPath $SummaryFile)
    $checks.Add((New-EvidenceCheckRow -Name "summary JSON exists" -Status $(if ($summaryExists) { "OK" } else { "FAIL" }) -Evidence $SummaryFile -Comment "Generated by exfiltracion_v3.ps1.")) | Out-Null
    ...
    $checks.Add((New-EvidenceCheckRow -Name "ZIP exists" -Status $(if ($zipExists) { "OK" } else { "FAIL" }) -Evidence $ZipFile -Comment "Controlled archive should remain available when KeepArtifacts is true.")) | Out-Null
    $checks.Add((New-EvidenceCheckRow -Name "ZIP copy exists" -Status $(if ($copyExists) { "OK" } else { "FAIL" }) -Evidence $CopyFile -Comment "Local copy documents staging/copy behavior.")) | Out-Null
    $checks.Add((New-EvidenceCheckRow -Name "SHA256 present" -Status $(if ($LocalSHA256 -match '^[0-9a-fA-F]{64}$') { "OK" } else { "FAIL" }) -Evidence $LocalSHA256 -Comment "Hash documents file integrity, not encryption.")) | Out-Null
    ...
    $checks.Add((New-EvidenceCheckRow -Name "HTTP status 2xx" -Status $(if ($httpOk) { "OK" } else { "FAIL" }) -Evidence $HttpStatus -Comment "Receiver must confirm controlled upload.")) | Out-Null
    $checks.Add((New-EvidenceCheckRow -Name "UploadSucceeded True" -Status $(if ($UploadSucceeded -match '(?i)^true$') { "OK" } else { "FAIL" }) -Evidence $UploadSucceeded -Comment "exfiltracion_v3.ps1 summary must confirm upload.")) | Out-Null
    ...
    $checks.Add((New-EvidenceCheckRow -Name "ReceiverUrl matches" -Status $(if ($summaryReceiver -eq $ReceiverUrl) { "OK" } else { "FAIL" }) -Evidence $summaryReceiver -Comment ("Expected: {0}" -f $ReceiverUrl))) | Out-Null

    $ps4104Count = 0
    if ($null -ne $PowerShell4104 -and $PowerShell4104.PSObject.Properties.Name -contains "Count") {
        $ps4104Count = [int]$PowerShell4104.Count
    }
    $checks.Add((New-EvidenceCheckRow -Name "PowerShell 4104 upload evidence" -Status $(if ($ps4104Count -gt 0) { "OK" } else { "WARN" }) -Evidence ("Count={0}" -f $ps4104Count) -Comment "WARN means campaign may be OK but Velociraptor live detection cannot be validated.")) | Out-Null
...
function Write-Tec009EvidenceCheck {
    ...
    foreach ($check in $Checks) {
        Write-Step ("{0}: {1} | {2} | {3}" -f $check.Status, $check.Name, $check.Evidence, $check.Comment)
    }
}
...
function Write-CampaignSummary {
    ...
    foreach ($check in $script:Tec009EvidenceCheck) {
        $lines.Add(("{0} | {1} | Evidence={2} | Comment={3}" -f $check.Status, $check.Name, $check.Evidence, $check.Comment)) | Out-Null
    }
...
$tec009Warnings = @($script:Tec009EvidenceCheck | Where-Object { $_.Status -eq "WARN" }).Count
