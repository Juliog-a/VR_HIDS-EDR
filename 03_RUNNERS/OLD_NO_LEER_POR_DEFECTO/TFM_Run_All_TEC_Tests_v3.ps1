...
<#
Candidate runner v3 for P4/P3 clean iteration.

Changes:
- Uses candidate TEC-007 scripts with aligned CSV/list paths.
- Validates that .aes files exist before restoring TEC-007.
- Uses candidate TEC-009 script to include Compress-Archive + Copy-Item + controlled HTTP upload.
...
param(
    [string]$Tec009ReceiverUrl = "http://127.0.0.1:8000/upload",
    [bool]$Tec009EnableUpload = $true,
    [int]$Tec009TimeoutSec = 30,
    [string]$Tec009ZipName = "tec009_controlled_archive.zip",
    [bool]$Tec009KeepArtifacts = $true
)
...
$ExfilScript = Join-Path $ScriptsCandidate "exfiltracion_v3.ps1"
...
$LogFile = Join-Path $LogDir ("TFM_TEC_Run_CANDIDATE_v3_{0}.log" -f (Get-Date -Format "yyyyMMdd_HHmmss"))
...
Write-Step "Starting candidate campaign v3 P4/P3"
Write-Step "Log: $LogFile"
Write-Step ("TEC-009 ReceiverUrl: {0}" -f $Tec009ReceiverUrl)
Write-Step ("TEC-009 EnableUpload: {0}" -f $Tec009EnableUpload)
...
Start-Test -Tec "TEC-009" -Name "Staging / Archive / Controlled HTTP Transfer"
Restore-DumbLab -Mode "Exfiltracion"
Assert-DumbLabSafe
$tec009EnableLiteral = if ($Tec009EnableUpload) { '$true' } else { '$false' }
$tec009KeepLiteral = if ($Tec009KeepArtifacts) { '$true' } else { '$false' }
Invoke-PSFileIfExists -ScriptPath $ExfilScript -Description "Controlled staging/archive plus HTTP transfer" -Arguments ('-WorkDir "{0}" -OutputDir "{1}" -ReceiverUrl "{2}" -EnableUpload:{3} -TimeoutSec {4} -ZipName "{5}" -KeepArtifacts:{6}' -f $DumbLab, (Join-Path $ExfilDir "staging_controlled"), $Tec009ReceiverUrl, $tec009EnableLiteral, $Tec009TimeoutSec, $Tec009ZipName, $tec009KeepLiteral)
Start-Sleep -Seconds $ShortPauseSeconds
Restore-DumbLab -Mode "Exfiltracion"
...
Restore-DumbLab -Mode "All"
Write-Step "Candidate campaign v3 finished"
