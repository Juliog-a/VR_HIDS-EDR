...
<#
Candidate runner v6 for TFM Velociraptor validation.
...
function Test-Tec009Evidence {
    ...
    $checks += New-EvidenceCheckRow -CheckName "PowerShell 4104 upload evidence" -Status $(if ($ps4104Count -gt 0) { "OK" } else { "WARNING" }) -Expected "Count greater than 0" -Found ("Count={0}" -f $ps4104Count) -Evidence ("Count={0}" -f $ps4104Count) -Comment "WARNING means campaign may be OK but Velociraptor live detection cannot be validated."
