...
function Read-VelociraptorHits {
    ...
    if (-not (Test-Path -LiteralPath $JsonlPath)) {
        Write-Log ("JSONL no disponible: {0}" -f $JsonlPath) "WARN"
        return @()
    }

    $lines = @(Get-Content -LiteralPath $JsonlPath -ErrorAction Stop | Where-Object { $_.Trim() -ne "" })
...
function Write-TextReport {
    ...
    $statusGroups = @($Script:Results | Group-Object Status | Sort-Object Name)
    $profileGroups = @($VrHits | Group-Object Profile | Sort-Object Count -Descending)
    $fpGroups = @($VrHits | Group-Object FP_ID | Sort-Object Count -Descending)
    $detGroups = @($VrHits | Group-Object DetectionName | Sort-Object Count -Descending)
    ...
    [System.IO.File]::WriteAllLines($VrHitsTxt, $hitLines, [System.Text.Encoding]::UTF8)
}
...
$Script:Results | Export-Csv -LiteralPath $SummaryCsv -NoTypeInformation -Encoding UTF8

if ($vrHits.Count -gt 0) {
    $vrHits | Export-Csv -LiteralPath $VrHitsCsv -NoTypeInformation -Encoding UTF8
} else {
    [System.IO.File]::WriteAllText($VrHitsCsv, "RunId,EventTime,MatchedByTime,MatchedByRunId,FP_ID,Profile,TEC,DetectionName,Severity,Confidence,Artifact,Source,PotentialFP,EvidenceExcerpt`r`n", [System.Text.Encoding]::UTF8)
}
...
$summaryObject = [pscustomobject]@{
    ...
    VelociraptorHits = [pscustomobject]@{
        Total = $vrHits.Count
        ByProfile = @($vrHits | Group-Object Profile | Sort-Object Count -Descending | Select-Object Name, Count)
        ByFP_ID = @($vrHits | Group-Object FP_ID | Sort-Object Count -Descending | Select-Object Name, Count)
        ByTEC = @($vrHits | Group-Object TEC | Sort-Object Count -Descending | Select-Object Name, Count)
        ByDetectionName = @($vrHits | Group-Object DetectionName | Sort-Object Count -Descending | Select-Object Name, Count)
    }
